            ...
            if (!compiled)
            {
                ...
                if (infoLen)
                {
                    ...
                    if (buf)
                    {
                        ...
                        EAGLEEYE_LOGE("GLUtils::LoadShader Could not compile shader %d:\n%s\n", shaderType, buf);
                        free(buf);
                    }
...
GLuint GLUtils::CreateProgram(const char *pVertexShaderSource, const char *pFragShaderSource, GLuint &vertexShaderHandle, GLuint &fragShaderHandle)
{
            ...
            if (linkStatus != GL_TRUE)
            {
                ...
                if (bufLength)
                {
                    ...
                    if (buf)
                    {
                        ...
                        EAGLEEYE_LOGE("GLUtils::CreateProgram Could not link program:\n%s\n", buf);
                        free(buf);
                    }
                }
    ...
    EAGLEEYE_LOGD("GLUtils::CreateProgram program = %d", program);
	return program;
}
...
GLuint GLUtils::CreateProgramWithFeedback(const char *pVertexShaderSource, const char *pFragShaderSource, GLuint &vertexShaderHandle, GLuint &fragShaderHandle, GLchar const **varying, int varyingCount)
{
        ...
        if (program)
        {
            ...
            if (linkStatus != GL_TRUE)
            {
                ...
                if (bufLength)
                {
                    ...
                    {
                        glGetProgramInfoLog(program, bufLength, NULL, buf);
                        EAGLEEYE_LOGE("GLUtils::CreateProgramWithFeedback Could not link program:\n%s\n", buf);
                        free(buf);
                    }
                }
                glDeleteProgram(program);
    ...
    EAGLEEYE_LOGD("GLUtils::CreateProgramWithFeedback program = %d", program);
    return program;
}

void GLUtils::DeleteProgram(GLuint &program)
{
    EAGLEEYE_LOGD("GLUtils::DeleteProgram");
    if (program)
    {
        glUseProgram(0);
        glDeleteProgram(program);
...
void GLUtils::CheckGLError(const char *pGLOperation)
{
    ...
    {
        EAGLEEYE_LOGE("GLUtils::CheckGLError GL Operation %s() glError (0x%d)\n", pGLOperation, error);
    }
