...
#include "eagleeye/engine/math/arm/elementwise_naive_impl.h"
#include "eagleeye/engine/math/arm/elementwise_common_broadcast.h"
...
template <>
void elementwise_add_broadcast_1c<float>(const float* dinx,
                                      const float* diny,
                                      float* dout,
                                      int batch,
                                      int num) {
...
#pragma omp parallel for
  for (int i = 0; i < batch; ++i) {
    ...
    for (int k = 0; k < cnt; ++k) {
      ...
      float32x4_t rb0 = vld1q_f32(diny_ptr);
      float32x4_t rb1 = vld1q_f32(diny_ptr + 4);
      float32x4_t rb2 = vld1q_f32(diny_ptr + 8);
      float32x4_t rb3 = vld1q_f32(diny_ptr + 12);

      din0 = vaddq_f32(din0, rb0);
      din1 = vaddq_f32(din1, rb1);
      din2 = vaddq_f32(din2, rb2);
      din3 = vaddq_f32(din3, rb3);
...
template <>
void elementwise_add_broadcast_n1<float>(const float* dinx,
                                      const float* diny,
                                      float* dout,
                                      int batch,
                                      int num) {
...
template <>
void elementwise_sub_broadcast_1c<float>(const float* dinx,
                                      const float* diny,
                                      float* dout,
                                      int batch,
                                      int num) {
#pragma omp parallel for
  for (int i = 0; i < batch; ++i) {
      ...
      for (int k = 0; k < cnt; ++k) {
        ...
        din0 = vsubq_f32(din0, rb0);
        din1 = vsubq_f32(din1, rb1);
        din2 = vsubq_f32(din2, rb2);
        din3 = vsubq_f32(din3, rb3);
...
template <>
void elementwise_sub_broadcast_n1<float>(const float* dinx,
                                      const float* diny,
                                      float* dout,
                                      int batch,
                                      int num) {
...
template <>
void elementwise_mul_broadcast_1c<float>(const float* dinx,
                                      const float* diny,
                                      float* dout,
                                      int batch,
                                      int num) {
#pragma omp parallel for
  for (int i = 0; i < batch; ++i) {
      ...
      for (int k = 0; k < cnt; ++k) {
        ...
        din0 = vmulq_f32(din0, rb0);
        din1 = vmulq_f32(din1, rb1);
        din2 = vmulq_f32(din2, rb2);
        din3 = vmulq_f32(din3, rb3);
...
template <>
void elementwise_mul_broadcast_n1<float>(const float* dinx,
                                      const float* diny,
                                      float* dout,
                                      int batch,
                                      int num) {
...
template <>
void elementwise_div_broadcast_1c<float>(const float* dinx,
                                      const float* diny,
                                      float* dout,
                                      int batch,
                                      int num) {
...
#ifdef __aarch64__
        din0 = vdivq_f32(din0, rb0);
        din1 = vdivq_f32(din1, rb1);
        din2 = vdivq_f32(din2, rb2);
        din3 = vdivq_f32(din3, rb3);
#else
        din0 = div_ps(din0, rb0);
        din1 = div_ps(din1, rb1);
        din2 = div_ps(din2, rb2);
        din3 = div_ps(din3, rb3);
#endif
...
template <>
void elementwise_div_broadcast_n1<float>(const float* dinx,
                                      const float* diny,
                                      float* dout,
                                      int batch,
                                      int num) {
...
template <>
void elementwise_pow_broadcast_1c<float>(const float* dinx,
                                      const float* diny,
                                      float* dout,
                                      int batch,
                                      int num) {
  naive_elementwise_op_broadcast_1c<float, naive_pow<float>>(
      dinx, diny, dout, batch, num);
}

template <>
void elementwise_pow_broadcast_n1<float>(const float* dinx,
                                      const float* diny,
                                      float* dout,
                                      int batch,
                                      int num) {
  naive_elementwise_op_broadcast_n1<float, naive_pow<float>>(
      dinx, diny, dout, batch, num);
}
