...
#include "eagleeye/common/EagleeyeDefines.h"
#include <algorithm>
#include <limits>
#include <math.h> 
#include <arm_neon.h>
  ...
  if (global_pooling) {
    if (pooling_type == "max") {  // Pooling_max
      for (int n = 0; n < num; ++n) {
        ...
        EAGLEEYE_PARALLEL_BEGIN(c, tid, chout) {
      ...
      EAGLEEYE_LOGE("unsupported pooling type: %s", pooling_type.c_str());
    }
  } else {
    for (int ind_n = 0; ind_n < num; ++ind_n) {
      EAGLEEYE_PARALLEL_BEGIN(ind_c, tid, chin) {
