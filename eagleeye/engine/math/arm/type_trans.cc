...
#include "eagleeye/engine/math/arm/saturate.h"
#include <arm_neon.h>
#include <string.h>
#include <vector>
...
#pragma omp parallel for
  for (int j = 0; j < loop_size; ++j) {
    ...
    const float* din_c = din + j * inner_size;
    unsigned char* dout_c = dout + j * inner_size;
    if (cnt > 0) {
      int cnt_loop = cnt;
      const float* din_ptr = din_c;
      unsigned char* dout_ptr = dout_c;
#ifdef __aarch64__
      float32x4_t vmax = vdupq_n_f32(0.0);
      asm volatile(
          "ldp q0, q1, [%[in]], #32                           \n"
          "ldp q2, q3, [%[in]], #32                   \n"
          "0:                                         \n" /* main loop */
    ...
    const float* din_r = din_c + 16 * cnt;
    unsigned char* dout_r = dout_c + 16 * cnt;
    for (int i = 0; i < remain; ++i) {
      dout_r[i] = saturate_cast<uint8_t>(roundf(inv_scale * din_r[i]));
    }
  }
}
