...
void CalcPaddingAndOutputSize(const index_t *input_shape,
                              ...
                              int *padding_size) {
  EAGLEEYE_CHECK(dilations[0] > 0 && dilations[1] > 0,
             "Invalid dilations, must >= 1");
  EAGLEEYE_CHECK((dilations[0] == 1 || strides[0] == 1) &&
      (dilations[1] == 1 || strides[1] == 1),
             "If dilations > 1, strides should be 1");
  EAGLEEYE_CHECK_NOTNULL(output_shape);
  EAGLEEYE_CHECK_NOTNULL(padding_size);
  ...
  */
  ...
  switch (padding) {
    ...
    case FULL:
      ...
      break;
    default:EAGLEEYE_CHECK(false, "Unsupported padding type: ", padding);
  }
...
void CalcDeconvShape_TF(const std::vector<index_t> &input_shape,
  ...
  switch (padding_type) {
    ...
    case SAME:
      ...
      break;
    default:EAGLEEYE_CHECK(false, "Unsupported padding type: ", padding_type);
  }

  EAGLEEYE_CHECK(expected_input_height == in_height,
             expected_input_height, "!=", in_height);
  EAGLEEYE_CHECK(expected_input_width == in_width,
...
//     EAGLEEYE_CHECK(output_shape->size() == 4,
