...
/* Image Signal */
template<class T>
ImageSignal<T>::ImageSignal(Matrix<T> data,char* name)
				:BaseImageSignal(TypeTrait<T>::type,TypeTrait<T>::size),
				img(data)
{
...
template<class T>
typename ImageSignal<T>::DataType ImageSignal<T>::getDataWithId(std::string id, MetaData& mm, bool deep_copy){
	// refresh data
	if(this->m_link_node != NULL){
		this->m_link_node->refresh();
	}
	if(this->getSignalCategory() == SIGNAL_CATEGORY_IMAGE){
		// SIGNAL_CATEGORY_IMAGE
		EAGLEEYE_LOGE("Dont support get data with ID for SIGNAL_CATEGORY_IMAGE");
		mm = MetaData();
		return Matrix<T>();
	}
